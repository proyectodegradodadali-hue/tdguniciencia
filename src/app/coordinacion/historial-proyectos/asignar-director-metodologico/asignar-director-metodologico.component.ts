...
@Component({
  selector: 'app-asignar-director-metodologico',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FormsModule, AlertaInformativoComponent],
  templateUrl: './asignar-director-metodologico.component.html',
  styleUrl: './asignar-director-metodologico.component.css'
})
export class AsignarDirectorMetodologicoComponent {
  ...
  abrirModal(modo: any, datos: any) {
    ...
    this.consultarDocentesMetodologicos();
    this.modo = modo;
    if (modo === 'editar' && datos) {
      if (datos.codigoDirectorMetodologico) {
        this.docentesSeleccionados = [{
          codigoEstudiante: datos.codigoDirectorMetodologico,
          nombreUsuario: datos.directorMetodologico
        }];
      }else{
        this.docentesSeleccionados = [];
      }
  ...
  async handleSave() {
    ...
    const nuevo = {
      ...this.datosEdicion,
      codigoDirectorMetodologico: this.docentesSeleccionados[0].codigoEstudiante,
      directorMetodologico: this.docentesSeleccionados[0].nombreUsuario,
    };
  ...
  docentesMetodologicos: any[] = [];
  docentesMetodologicosAll: any[] = [];
  docentesSeleccionados: any[] = [];
  searchText: string = '';
  subcripcion: any;

  consultarDocentesMetodologicos() {
    const path = `Docentes`;
    ...
    this.subcripcion = this._firestore.getDocumentsQueryChanges(path, q, extras).subscribe(res => {
      this.docentesMetodologicosAll = res;
      this.docentesMetodologicos = res;
      this.applyFilters();
      console.log('docentesMetodologicos', this.docentesMetodologicos);
    });
  }
  ...
  applyFilters() {
    let filtered = [...this.docentesMetodologicosAll];
    ...
    this.docentesMetodologicos = filtered;
  }
